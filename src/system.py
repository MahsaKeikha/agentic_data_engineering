...
SYSTEM_ID, SYSTEM_NAME, VERSION = "F33", "Agentic Data Engineering", "0.2.1"
...
def run_system(case: Dict[str, Any], approve: bool = False) -> Dict[str, Any]:
    s = State(case)
    s.record("data_engineering_orchestrator", "run started", {"system_id": SYSTEM_ID, "version": VERSION})
    ...
    for evidence in case.get("evidence", []):
        s.evidence.append({
            ...
            "status": str(evidence.get("status", "supplied")),
        })
    ...
    s.record("data_engineering_orchestrator", "handoff gate evaluated", {"approve": approve, "blockers": blockers, "status": status})
    return {
        "system_id": SYSTEM_ID,
        ...
        "risks": s.risks,
        "recommendation": "Resolve data engineering blockers." if blockers else "Data engineering package is ready for human review.",
        "status": status,
        "trace": s.trace,
    }
